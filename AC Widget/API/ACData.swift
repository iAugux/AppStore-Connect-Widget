...
extension ACData {
    func changeCurrency(to outputCurrency: Currency) -> ACData {
        let newEntries: [ACEntry] = entries.map { entry -> ACEntry in
        ...
        return ACData(entries: newEntries, currency: outputCurrency, apps: apps)
    }
}
...
extension ACData {
    ...
    func getRawData(for type: InfoType, lastNDays: Int, filteredApps: [ACApp] = []) -> [(Float, Date)] {
        ...
        return result.fillZeroLastDays(lastNDays, latestDate: latestReportingDate())
    }
...
extension ACData {
    ...
    private static func createMockData(_ days: Int, largeValues: Bool = false) -> ACData {
        ...
        Date(timeIntervalSinceNow: -86_400).getLastNDates(days).forEach { day in
